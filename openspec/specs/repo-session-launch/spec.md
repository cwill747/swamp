...
Launch SHALL generate a dashboard-first layout for bare/git-wt-style repositories and a worktree-focused layout for normal repositories.

#### Scenario: Bare repository layout
- **WHEN** the target repository is bare or uses a bare worktree layout
- **THEN** the generated session starts with a focused dashboard tab followed by worktree tabs

#### Scenario: Normal repository layout
- **WHEN** the target repository is not bare
- **THEN** the generated session focuses the first worktree tab directly
